...
export default function TradingPageRedesign() {
  ...
  const trade = trpc.family.ledger.trade.useMutation({
    onSuccess: () => {
      toast.success("تم اعتماد الصفقة وتحديث الأصل ومتوسط التكلفة في معاملة واحدة.");
  ...
  return <DashboardLayout><main className="mx-auto max-w-7xl space-y-6" dir="rtl">
    ...
    <ConfirmDialog
      ...
      title={`تأكيد اعتماد صفقة ${side === "buy" ? "شراء" : "بيع"}`}
      description={`أنت على وشك اعتماد صفقة ${side === "buy" ? "شراء" : "بيع"} لـ ${quantity} وحدة من أداة "${selectedInstrument?.name}" عبر حساب "${selectedAccount?.name}". سيتم قيد العملية في دفتر الأستاذ وتحديث متوسط التكلفة والأصول.`}
      confirmText={`تأكيد اعتماد صفقة الـ ${side === "buy" ? "شراء" : "بيع"}`}
